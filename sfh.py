...
class double_powerlaw_no_age:
    ...
    star formation rate.
        ...
        SFR(t) = 10**a * ((t/tau)**b + (t/tau)**-c)**-1
    '''
    def __init__(self, tau=-2.5, a=1.5, b=2., c=1., tau_lims=[-3., 1.],
                 a_lims=[-1., 5.], b_lims=[0., 5.], c_lims=[0., 5.],
                 tau_delta=0.2, a_delta=0.7, b_delta=0.5, c_delta=0.3):
        ''' Initialize this class
    ...
    def get_params(self):
        ''' Return current parameters '''
        return [self.tau, self.a, self.b, self.c]
    ...
    def prior(self):
        ...
        b_flag = (self.b > self.b_lims[0])*(self.b < self.b_lims[1])
        c_flag = (self.c > self.c_lims[0])*(self.c < self.c_lims[1])
        return tau_flag * a_flag * b_flag * c_flag
    ...
    def plot(self, ax, color=[238/255., 90/255., 18/255.], alpha=0.2):
        ''' Plot SFH for given set of parameters '''
        t = np.logspace(self.age_lims[0], self.age_lims[1])
        sfr = self.evaluate(t)
        ax.plot(t, sfr, color=color, alpha=alpha)
...
class empirical:
    ...
    def __init__(self, mass=9., mass_lims=[6., 12.],
        ...
        weight = 1. / (np.array(self.ages) - 6.2)
        weight /= weight.sum()
        for num, wei in zip(self.nums, weight[:-1]):
            setattr(self, 'frac_' + str(num), wei)
            setattr(self, 'frac_' + str(num) + '_lims', [0., 1.])
            setattr(self, 'frac_' + str(num) + '_delta', wei/4.)
        total = 0
        for num in self.nums:
            total += getattr(self, 'frac_' + str(num))
    ...
    def evaluate(self, t):
        ...
        msfr : numpy array (1 dim)
            Star formation rate at given time in time array
        '''
        v = self.get_params() + [self.lastbin]
        mass = 10**v[0]
